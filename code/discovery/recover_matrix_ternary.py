...
def initializeTableMatrices(sourceTable, tableDfs, primaryKey, foreignKeys, use_bert=False):
    from knowledge_graph import knowledge_graph

    def check_with_kg(entity, attr_name, attr_val):
        if entity in knowledge_graph:
            return attr_val in knowledge_graph[entity].get(attr_name, [])
        return False
            ...
            for qCol in sourceTable.columns:
                ...
                if qKeyVal in df[commonKey].values:
                    ...
                    for valIndx, val in enumerate(dfVals):
                        if pd.isna(val) and pd.isna(sourceVal):
                            dfMatrix[qKey][valIndx][j] = 1.0
                        elif not pd.isna(val) and not pd.isna(sourceVal):
                            if use_bert:
                                sim = bert_similarity(val, sourceVal)
                                if sim > 0.85:
                                    dfMatrix[qKey][valIndx][j] = 1.0
                                elif sim > 0.5:
                                    dfMatrix[qKey][valIndx][j] = 0.5
                                else:
                                    dfMatrix[qKey][valIndx][j] = -1.0
                            else:
                                if val == sourceVal:
...
def getDLMatrices(sourceTable, tableDfs, primaryKey, foreignKeys, startNode=None, use_bert=False):
    '''
    ...
    tableMatrices = initializeTableMatrices(sourceTable, tableDfs, primaryKey, foreignKeys, use_bert)
...
def main(benchmark, sourceTableName='source', use_bert=False):
    FILEPATH = os.path.join("datasets", benchmark)
    timesStats = {}
    startTime = time.time()
    # ===== REAL Data Lake ====
    datasets = glob.glob(os.path.join(FILEPATH, "datalake", "*.csv"))
    foundPaths = []
    print("\t=========== Source Table: %s =========== " % (sourceTableName))
    sourceTable = pd.read_csv(os.path.join(FILEPATH, "queries", sourceTableName))
    # ==== TPTR Datalake
    ...
    print("CANDIDATE TABLES LOADED:", list(tableDfs.keys()))
    ...
    finalTableDfs, tableMatrices, traversedTables, correctVals, matrixInitTime, matTraverseTime = getDLMatrices(sourceTable, finalTableDfs, primaryKey, foreignKeys, use_bert=use_bert)
    print("TRAVERSED TABLES:", traversedTables)
