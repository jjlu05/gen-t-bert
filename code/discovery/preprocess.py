...
def getTableDfs(benchmark, datasets, sourceTableName):
    candidateTablePath = os.path.join("results_candidate_tables", benchmark, "candidateTables.json")
    
    if not os.path.isfile(candidateTablePath): return None
    with open(candidateTablePath) as json_file:
        candidateTableDict = json.load(json_file)[sourceTableName]

    print("Expected tables:", candidateTableDict.keys())  # ✅ Now candidateTableDict is defined
    print("Files available:", [os.path.basename(p) for p in datasets])
    tableDfs = {}
    numAdditional = 0
