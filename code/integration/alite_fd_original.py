...
def loadCandidateTables(benchmark, sourceTableName, candidateTableDict, performProjSel):
    BASE_DATA_DIR = os.path.join("datasets", "santos_large_tptr" if "_groundtruth" in benchmark else benchmark)
    query_path = os.path.join(BASE_DATA_DIR, "queries", sourceTableName)
    sourceTable = pd.read_csv(query_path)
    primaryKey = sourceTable.columns[0]

    dataLakeDir = os.path.join(BASE_DATA_DIR, "datalake")
    datasets = glob.glob(os.path.join(dataLakeDir, "*.csv"))

    print("Imported %d raw candidates" % len(candidateTableDict))
    tableDfs = {}
    for path in datasets:
        tableName = os.path.basename(path)
        if tableName in candidateTableDict and tableName != sourceTableName:
            table_df = pd.read_csv(path)
            if len(candidateTableDict[tableName]) > 0:
                print("RENAME COLUMNS:", tableName, candidateTableDict[tableName])
            ...
            for col in similar_qCols:
                try:
                    table_df[col] = table_df[col].astype(sourceTable[col].dtypes.name)
                except:
                    table_df = table_df.dropna()
                    try:
                        table_df[col] = table_df[col].astype(sourceTable[col].dtypes.name)
                    except:
                        table_df = table_df.drop(col, axis=1)
            tableDfs[tableName] = table_df

    finalTableDfs = tableDfs
    if performProjSel:
        foreignKeys = [col for col in sourceTable.columns if 'key' in col and col != primaryKey]
...
def main(benchmark, sourceTableName, candidateTablesDict, performProjSel, timeout):
    ...
    print("%d Candidate Tables: " % len(candidate_tables), list(candidate_tables.keys()))
    noCandidates = not bool(candidate_tables)
    timed_out = False
    numOutputVals = 0
    ...
    output_dir = "output_tables_projSel" if performProjSel else "output_tables"
    output_path = os.path.join(output_dir, benchmark)
    ...
    if result_FD is None:
        ...
        result_FD.to_csv(os.path.join(output_path, sourceTableName), index=False)
